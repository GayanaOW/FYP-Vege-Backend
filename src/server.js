...
app.post("/api/admin/predictions/generate/potatoes", adminAuth, async (req, res) => {
  try {
    const horizon = Number(req.query.horizon || 7);
    const target = "Import Potatoes";
    const modelName = "SARIMAX";
    const runId = `potatoes_${Date.now()}`;

    // get last actual date (to align forecast start date)
    ...
    const pyScript = path.join(__dirname, "python/forecast_potatoes.py");

    const payload = await new Promise((resolve, reject) => {
      execFile(
        "python",
        [pyScript, String(horizon), String(lastActualDate)],
        ...
        (err, stdout, stderr) => {
          ...
          try {
            ...
            console.error("PY STDOUT:", stdout?.slice(0, 500));
            console.error("PY STDERR:", stderr);
    ...
    const values = payload.forecast.map((r) => [
      target,
      r.date,
      round1(r.predicted),
      round1(r.lower),
    ...
    await logAdminAction({
      ...
      status: "success",
      details: `Generated ${values.length} forecast rows for Import Potatoes using ${modelName} (horizon=${horizon})`,
    });

    return res.json({
      ok: true,
      message: "Predictions generated and saved",
      target,
      model: modelName,
      runId,
      horizon,
    ...
    await logAdminAction({
      ...
      action: "generate_forecast",
      status: "failed",
      details: `Failed to generate forecast for Import Potatoes (horizon=${req.query.horizon || 7})`,
    });
    console.error(e);
    return res.status(500).json({ ok: false, message: "Failed to generate predictions" });
  }
});
...
app.post("/api/admin/predictions/generate/big-onions", adminAuth, async (req, res) => {
  try {
    const horizon = Number(req.query.horizon || 7);
    const target = "Import Big Onions";
    const modelName = "VAR_LSTM_HYBRID";
    const runId = `big_onions_${Date.now()}`;

    // latest actual date for this vegetable
    const [[row]] = await db.query(
      `SELECT DATE_FORMAT(MAX(date), '%Y-%m-%d') AS lastDate
       FROM train_ready_daily
    ...
    let args = [String(horizon), String(lastActualDate)];

    const pyScript = path.join(__dirname, "python", "forecast_var_lstm.py");
    ...
    const payload = await new Promise((resolve, reject) => {
        execFile(
          ...
          (err, stdout, stderr) => {
            ...
            if (!parsed) {
              console.error("PY STDOUT:", stdout);
              return reject(new Error("Invalid python output"));
            }
    ...
    await logAdminAction({
      ...
      action: "generate_forecast",
      status: "success",
      details: `Generated ${values.length} forecast rows for ${target} using ${modelName} (horizon=${horizon})`,
    });
    ...
    await logAdminAction({
      ...
      action: "generate_forecast",
      status: "failed",
      details: `Failed to generate forecast for Import Big Onions (horizon=${req.query.horizon || 7})`,
    });
...
app.post("/api/admin/predictions/generate/red-onions", adminAuth, async (req, res) => {
  try {
    const horizon = Number(req.query.horizon || 7);
    const target = "Import Red Onions";
    const modelName = "SARIMAX";
    const runId = `red_onions_${Date.now()}`;
    ...
    // call python forecast script
    const pyScript = path.join(__dirname, "python/forecast_red_onions.py");
    ...
    await logAdminAction({
      ...
      action: "generate_forecast",
      status: "success",
      details: `Generated ${values.length} forecast rows for Import Red Onions using ${modelName} (horizon=${horizon})`,
    });
    ...
    await logAdminAction({
      ...
      action: "generate_forecast",
      status: "failed",
      details: `Failed to generate forecast for Import Red Onions (horizon=${req.query.horizon || 7})`,
    });
    console.error(e);
    return res.status(500).json({ ok: false, message: "Failed to generate predictions" });
  }
...
app.post("/api/admin/predictions/generate/chillies", adminAuth, async (req, res) => {
  try {
    const horizon = Number(req.query.horizon || 7);
    const target = "Import Chillies";
    const modelName = "SARIMAX";
    const runId = `chillies_${Date.now()}`;
    ...
    // call python forecast script
    const pyScript = path.join(__dirname, "python/forecast_chillies.py");
    ...
    await logAdminAction({
      ...
      action: "generate_forecast",
      status: "success",
      details: `Generated ${values.length} forecast rows for Import Chillies using ${modelName} (horizon=${horizon})`,
    });
    ...
    await logAdminAction({
      ...
      action: "generate_forecast",
      status: "failed",
      details: `Failed to generate forecast for Import Chillies (horizon=${req.query.horizon || 7})`,
    });
    console.error(e);
    return res.status(500).json({ ok: false, message: "Failed to generate predictions" });
  }
